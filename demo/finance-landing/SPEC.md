...
- **Câu hỏi gạn lọc:** mức đầu tư dự kiến (trường trong form).
- **Lead scoring (server):** cộng điểm theo mức đầu tư + tính hợp lệ liên hệ → phân loại:
...
export function scoreLead(answers: LeadAnswers): LeadScore {
  let points = 0;
  points += INVESTMENT_POINTS[answers.investmentRange] ?? 0;
  if (answers.validPhone) points += 15;
...
- Form: bản gọn 5 trường (tên, email, SĐT, mức đầu tư, consent) + honeypot ẩn.
